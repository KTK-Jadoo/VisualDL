 ...
 * limitations under the License.
 * =============================================================================
 */me.wasm.wasmPaths="https://cdn.jsdelivr.net/npm/onnxruntime-web@1.22.0/dist/",me.wasm.numThreads=1;let hc=null,mc=!1;const Yt=new Uint8Array(256*4);(function(){const t=[[.267,.005,.329],[.283,.141,.458],[.254,.265,.53],[.207,.372,.553],[.164,.471,.558],[.128,.567,.551],[.135,.659,.518],[.267,.748,.441],[.478,.821,.318],[.741,.873,.15],[.993,.906,.144],[.993,.746,.239],[.988,.552,.349],[.94,.364,.432],[.839,.19,.502],[.616,.04,.475]],r=t.length,i=(a,n,s)=>a+(n-a)*s;for(let a=0;a<256;a++){const n=a/255,s=n*(r-1),u=Math.floor(s),l=Math.min(r-1,u+1),d=s-u,c=i(t[u][0],t[l][0],d),f=i(t[u][1],t[l][1],d),m=i(t[u][2],t[l][2],d),g=a*4;Yt[g]=Math.round(c*255),Yt[g+1]=Math.round(f*255),Yt[g+2]=Math.round(m*255),Yt[g+3]=255}})();function gc(e,t,r,i=Yt){const a=new Uint8Array(t*r*4);for(let n=0;n<t*r;n++){const s=Math.max(0,Math.min(255,e[n]*255|0))*4,u=n*4;a[u]=i[s],a[u+1]=i[s+1],a[u+2]=i[s+2],a[u+3]=255}return a}function _c(e){const t=e.dims[1],r=e.dims[2],i=e.dims[3],a=new Array(t),n=new Float32Array(t);for(let u=0;u<t;u++){const l=new Float32Array(i*r);let d=0,c=1/0,f=-1/0;for(let g=0;g<r;g++)for(let _=0;_<i;_++){const b=e.data[(u*r+g)*i+_];l[g*i+_]=b,d+=b,b<c&&(c=b),b>f&&(f=b)}n[u]=d/(i*r);const m=f-c||1;for(let g=0;g<l.length;g++)l[g]=(l[g]-c)/m;a[u]=gc(l,i,r)}const s=[...Array(t).keys()].sort((u,l)=>n[l]-n[u]);return{rgba:a,topk:s.slice(0,Math.min(6,t))}}function em(e){const t=Math.max(...e),r=e.map(a=>Math.exp(a-t)),i=r.reduce((a,n)=>a+n,0)||1;return r.map(a=>a/i)}async function tm(e){try{hc=await jr.create(e,{executionProviders:["wasm"],graphOptimizationLevel:"all"}),mc=!0,postMessage({type:"ready"})}catch(t){postMessage({type:"error",payload:`Init failed: ${t}`})}}onmessage=async e=>{const{type:t,modelUrl:r,payload:i}=e.data||{};if(t==="init")return tm(r);if(t==="infer"){if(!mc)return;try{const a=new Uint8Array(i),n=new Float32Array(784);for(let k=0;k<784;k++){const v=a[k*4],w=a[k*4+1],S=a[k*4+2];n[k]=(.2126*v+.7152*w+.0722*S)/255}const s=new Pe("float32",n,[1,1,28,28]),u=await hc.run({input:s}),l=u.probs,d=u.conv1,c=u.conv2,f=gc(n,28,28),{rgba:m}=_c(d),{rgba:g,topk:_}=_c(c),b=em(Array.from(l.data));postMessage({type:"activations",payload:{inputRGBA:f,conv1RGBA:m,conv2RGBA:g,conv2TopK:_,probs:b}},[f.buffer,...m.map(k=>k.buffer),...g.map(k=>k.buffer)])}catch(a){postMessage({type:"error",payload:`Infer failed: ${a}`})}}}})();
